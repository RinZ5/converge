...
func TestBranchHandler_CreateBranch(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "capacity omitted defaults to unlimited",
			body:         `{"name": "Riverside"}`,
			mock:         &mockBranchService{addBranchResult: &branch.Branch{ID: 3, Name: "Riverside", Capacity: -1}},
			wantStatus:   http.StatusCreated,
			wantCalled:   true,
			wantName:     "Riverside",
			wantCapacity: -1,
...
func TestBranchHandler_UpdateBranchCapacity(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			path:         "/api/branches/1/capacity",
			body:         `{"capacity": 0}`,
			mock:         &mockBranchService{setCapacityErr: &shared.ValidationError{Msg: "capacity must be -1 for unlimited or a positive integer"}},
			wantStatus:   http.StatusBadRequest,
			wantCalled:   true,
			wantCapacity: 0,
		},
