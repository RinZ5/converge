...
func TestBranchService_AddBranch_UnlimitedCapacity_Allowed(t *testing.T) {
	store := new(mockStore)
	svc := NewService(store, slog.Default())

	expected := &Branch{ID: 4, Name: "Riverside", Capacity: -1}
	store.On("AddBranch", mock.Anything, "Riverside", -1).Return(expected, nil)

	b, err := svc.AddBranch(context.Background(), "Riverside", -1)
	assert.NoError(t, err)
	assert.Equal(t, expected, b)
}

func TestBranchService_AddBranch_ZeroCapacity_Rejected(t *testing.T) {
	store := new(mockStore)
	svc := NewService(store, slog.Default())

	_, err := svc.AddBranch(context.Background(), "Riverside", 0)
...
func TestBranchService_SetCapacity_UnlimitedCapacity_Allowed(t *testing.T) {
	store := new(mockStore)
	svc := NewService(store, slog.Default())

	store.On("SetCapacity", mock.Anything, 1, -1).Return(nil)
...
func TestBranchService_SetCapacity_ZeroCapacity_Rejected(t *testing.T) {
	store := new(mockStore)
	svc := NewService(store, slog.Default())
...
func TestBranchService_SetCapacity_BelowUnlimited_Rejected(t *testing.T) {
	store := new(mockStore)
	svc := NewService(store, slog.Default())

	err := svc.SetCapacity(context.Background(), 1, -2)
