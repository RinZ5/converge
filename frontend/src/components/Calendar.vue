<script setup lang="ts">
  ...
  const internalEvents = ref<EventInput[]>(props.modelValue || [])
  ...
  watch(
    () => props.modelValue,
    (val) => {
      if (val !== internalEvents.value) {
        internalEvents.value = val || []
      }
    }
  )
  ...
  const handleDateSelect = (selectInfo: DateSelectArg) => {
    ...
    internalEvents.value.push({
      id: generateEventId(),
      start,
      end,
      title: '',
    })
    emit('update:modelValue', internalEvents.value)
  }
  ...
  const confirmDelete = () => {
    if (eventToDelete.value) {
      internalEvents.value = internalEvents.value.filter((e) => e.id !== eventToDelete.value)
      emit('update:modelValue', internalEvents.value)
    }
    closeDeleteDialog()
  }
    ...
    >
      ...
      >
        ...
        >
          <div
            v-if="showDeleteDialog"
            class="bg-white rounded-2xl shadow-2xl border border-slate-200 p-6 w-full max-w-sm mx-4"
          >
